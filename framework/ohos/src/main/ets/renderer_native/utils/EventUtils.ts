...
export class EventUtils {
  ...
  static sendComponentEvent(ctx: NativeRenderContext, nodeId: number, eventName: string, params: any) {
    EventUtils.send(ctx, nodeId, eventName, params, false, false, EventType.EVENT_TYPE_COMPONENT)
  }

  static sendGestureEvent(ctx: NativeRenderContext, nodeId: number, eventName: string, params: any) {
    EventUtils.send(ctx, nodeId, eventName, params, true, true, EventType.EVENT_TYPE_GESTURE)
  }

  static sendCustomEvent(ctx: NativeRenderContext, nodeId: number, eventName: string, params: any, useCapture: boolean, useBubble: boolean) {
    EventUtils.send(ctx, nodeId, eventName, params, useCapture, useBubble, EventType.EVENT_TYPE_CUSTOM)
  }

  static sendRootEvent(rendererId: number, rootId: number, eventName: string, params: any) {
    let provider = NativeRenderProviderManager.getRenderProvider(rendererId)
    if (provider) {
      provider.dispatchEvent(rootId, rootId, eventName, params, false, false, EventType.EVENT_TYPE_ROOT)
    }
  }

  static send(ctx: NativeRenderContext, nodeId: number, eventName: string, params: any, useCapture: boolean, useBubble: boolean, eventType: EventType) {
